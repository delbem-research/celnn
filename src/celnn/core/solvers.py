...
def _store_result(
    ...
    metadata = _base_metadata(network, state=state)
...
def _solve_euler(
    ...
    state, trajectory_state, trajectory_output, result_times = (
...
def _solve_time_marching(
    ...
    for index in range(1, len(times)):
        ...
        state = np.asarray(advance(state, dt), dtype=network.dtype)
...
def _solve_ivp(
    ...
    if network.dtype != np.dtype(np.float64):
        raise SolverError(
            "The 'solve_ivp' solver requires a float64 CellularNetwork."
        )
    try:
    ...
    times = config.time_points()
    initial_state = network.state.astype(np.float64, copy=True)
    ...
    if config.return_trajectory:
        ...
        return SimulationResult(
            ...
            metadata=_base_metadata(network, state=final_state),
        )
